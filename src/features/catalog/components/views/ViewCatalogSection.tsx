...
interface ModalProps {
  ...
  onClose: () => void;
  title?: string;
  footer?: React.ReactNode; // 👈 Agregamos prop opcional para el footer
}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6">
      <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
        {/* Header */}
        {title && (
          <header className="flex items-center justify-between border-b border-gray-100 px-6 sm:px-8 py-5 shrink-0 bg-white">
        ...
        {/* Body con Scroll */}
        <main className="flex-1 overflow-y-auto px-6 sm:px-8 py-6 bg-slate-50/50">
          {children}
        </main>

        {/* Footer Fijo (Si se provee) */}
        {footer && (
          <footer className="border-t border-gray-100 bg-white px-6 sm:px-8 py-4 shrink-0 shadow-lg">
            {footer}
          </footer>
        )}
      </div>
